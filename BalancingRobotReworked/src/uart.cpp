...
void uart1_init(unsigned int baudrate)
{
    ...
    /* Set frame format: asynchronous, 8data, no parity, 1stop bit */   
    UCSR1C &= ~(_BV(UMSEL10) | _BV(UMSEL11));       //asynchronous mode
    UCSR1C &= ~(_BV(UPM10) | _BV(UPM11));           //no parity mode
    UCSR1C &= ~(_BV(USBS1));                      //1 stop-bit
    UCSR1C |= (_BV(UCSZ11) | _BV(UCSZ10));           //8-bits otaf da
    UCSR1B &= ~(_BV(UCSZ12));
}/* uart_init */
...
void uart2_init(unsigned int baudrate)
{
    ...
    /* Set frame format: asynchronous, 8data, no parity, 1stop bit */
    UCSR2C &= ~(_BV(UMSEL20) | _BV(UMSEL21));       //asynchronous mode
    UCSR2C &= ~(_BV(UPM20) | _BV(UPM21));           //no parity mode
    UCSR2C &= ~(_BV(USBS2));                      //1 stop-bit
    UCSR2C |= (_BV(UCSZ21) | _BV(UCSZ20));           //8-bits otaf da
    UCSR2B &= ~(_BV(UCSZ22));
}/* uart_init */
